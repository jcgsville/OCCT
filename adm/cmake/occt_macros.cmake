...
function (FILE_TO_LIST FILE_NAME FILE_CONTENT)
  ...
  if (EXISTS "${OCCT_SOURCE_DIR}/${FILE_NAME}")
    file (STRINGS "${OCCT_SOURCE_DIR}/${FILE_NAME}" LOCAL_FILE_CONTENT)
  endif()
...
function(FIND_FOLDER_OR_FILE FILE_OR_FOLDER_NAME RESULT_PATH)
  if (EXISTS "${OCCT_SOURCE_DIR}/${FILE_OR_FOLDER_NAME}")
    set (${RESULT_PATH} "${OCCT_SOURCE_DIR}/${FILE_OR_FOLDER_NAME}" PARENT_SCOPE)
  else()
...
function (OCCT_ORIGIN_AND_PATCHED_FILES RELATIVE_PATH SEARCH_TEMPLATE RESULT)
  file (GLOB ORIGIN_FILES "${OCCT_SOURCE_DIR}/${RELATIVE_PATH}/${SEARCH_TEMPLATE}")
...
macro (OCCT_INSTALL_FILE_OR_DIR BEING_INSTALLED_OBJECT DESTINATION_PATH)
  if (IS_DIRECTORY "${OCCT_SOURCE_DIR}/${BEING_INSTALLED_OBJECT}")
    install (DIRECTORY "${OCCT_SOURCE_DIR}/${BEING_INSTALLED_OBJECT}" DESTINATION  "${DESTINATION_PATH}")
  else()
    install (FILES     "${OCCT_SOURCE_DIR}/${BEING_INSTALLED_OBJECT}" DESTINATION  "${DESTINATION_PATH}")
  endif()
endmacro()

macro (OCCT_CONFIGURE_AND_INSTALL BEING_CONGIRUGED_FILE BUILD_NAME INSTALL_NAME DESTINATION_PATH)
  configure_file("${OCCT_SOURCE_DIR}/${BEING_CONGIRUGED_FILE}" "${BUILD_NAME}" @ONLY)
...
function (COLLECT_AND_INSTALL_OCCT_HEADER_FILES THE_ROOT_TARGET_OCCT_DIR THE_OCCT_BUILD_TOOLKITS THE_RELATIVE_PATH THE_OCCT_INSTALL_DIR_PREFIX)
  ...
  set (TEMPLATE_HEADER_PATH "${OCCT_SOURCE_DIR}/adm/templates/header.in")
...
macro (OCCT_COPY_FILE_OR_DIR BEING_COPIED_OBJECT DESTINATION_PATH)
  ...
  if (EXISTS "${OCCT_SOURCE_DIR}/${BEING_COPIED_OBJECT}")
    file (COPY "${OCCT_SOURCE_DIR}/${BEING_COPIED_OBJECT}" DESTINATION  "${DESTINATION_PATH}")
  endif()
endmacro()

macro (OCCT_CONFIGURE BEING_CONGIRUGED_FILE FINAL_NAME)
  configure_file("${OCCT_SOURCE_DIR}/${BEING_CONGIRUGED_FILE}" "${FINAL_NAME}" @ONLY)
endmacro()

macro (OCCT_ADD_SUBDIRECTORY BEING_ADDED_DIRECTORY)
  if (EXISTS "${OCCT_SOURCE_DIR}/${BEING_ADDED_DIRECTORY}/CMakeLists.txt")
    add_subdirectory (${OCCT_SOURCE_DIR}/${BEING_ADDED_DIRECTORY})
  else()
...
macro (FLEX_AND_BISON_TARGET_APPLY THE_PACKAGE_NAME RELATIVE_SOURCES_DIR)
  ...
  foreach (FLEX_FILE_INDEX RANGE ${SOURCE_FILES_FLEX_LEN})
    ...
    set (FLEX_BISON_TARGET_DIR "${OCCT_SOURCE_DIR}/${RELATIVE_SOURCES_DIR}/${THE_PACKAGE_NAME}")
    ...
    BISON_TARGET (Parser_${CURRENT_BISON_FILE_NAME} ${CURRENT_BISON_FILE} "${FLEX_BISON_TARGET_DIR}/${BISON_OUTPUT_FILE}"
                  COMPILE_FLAGS "-p ${CURRENT_BISON_FILE_NAME} -l -M ${OCCT_SOURCE_DIR}/${RELATIVE_SOURCES_DIR}/=")
